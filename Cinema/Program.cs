...
using (var scope = serviceProvider.CreateScope())
{
    ...
    var adminEmail = "david203018@yahoo.com";
    var adminUser = await userManager.FindByEmailAsync(adminEmail);
    if (adminUser == null)
    {
        adminUser = new ApplicationUser { UserName = adminEmail, Email = adminEmail, EmailConfirmed = true };

        var result = await userManager.CreateAsync(adminUser, "Gorgievski123#");

        var userId = await userManager.GetUserIdAsync(adminUser);

        await shoppingCart.Add(new Domain.DomainModels.ShoppingCart { UserId = userId });

        await shoppingCart.Add(new ShoppingCart { UserId = adminUser.Id });
